...
class SubSource[+Out, +Mat](delegate: scaladsl.SubFlow[Out, Mat, scaladsl.Source[Out, Mat]#Repr, scaladsl.RunnableGraph[Mat]]) {
   ...
   * input stream).
   */
  def mergeBack(breadth: Int): Source[Out, Mat] =
    new Source(delegate.mergeBack(breadth))
   ...
   *
   */
  def flatMapConcat[T](f: function.Function[Out, Source[T, _]]): SubSource[T, Mat] =
    new SubSource(delegate.flatMapConcat[T](x ⇒ f(x).asScala))
