...
If the ``groupBy`` operator emits more than this number then the stream cannot
continue: in order to write the new word to its new substream there needs to be
demand on that substream and ``mergeBack`` will not create this demand because
it cannot consume the resulting elements within its resource bounds. The
consequence is that the stream will deadlock.
